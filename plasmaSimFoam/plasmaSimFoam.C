	...
	else if (solutionDomain == "plasma")
	{
		while (runTime.run())
		{
		    ...
		    {		    
			    ...
			    forAll(composition.Y(), i)
			    {
			    	// Info << "print mspm().N(i) for " << composition.species()[i] << endl;
			    	// Info << mspm().N(i) << endl;
			    	// Info << "print mspm(), Y[i] for " << composition.species()[i] << endl;
			    	// Info << Y[i].boundaryField().types() << endl;
			    	// Info << "plasmaSimFoam"
					volScalarField specN
					(
						IOobject
